...
\subsection{Notebook 05 --- Level 3 Relevance Attribution Workflow}

\subsubsection*{Purpose}
Validate the rollout-based Level 3 attribution workflow on one real IEMOCAP
utterance, using the same example as Notebook 04.

\subsubsection*{Technical idea}
Notebook 05 documents the hybrid Level 3 construction separately from
LeGrad-HuBERT. For each Transformer layer, the layer matrix used by rollout is
not the raw attention map. It is built as
\[
G_l =
\mathrm{mean}_{\mathrm{heads}}\left(
\max\left(0,\frac{\partial s_{\hat{y}}}{\partial A_l}\odot A_l\right)
\right),
\]
where \(s_{\hat{y}}\) is the predicted-class score and \(A_l\) is the attention
map of layer \(l\). The matrices \(G_l\) are passed through residual-added,
row-normalized attention rollout to obtain a temporal rollout score. That score
is then multiplied by a lightweight classifier-head relevance term:
\[
R_{\mathrm{level3}} =
\mathrm{normalize}\left(
R_{\mathrm{rollout}} \odot R_{\mathrm{head}}
\right).
\]
This is the key distinction from Notebook 04: Level 3 uses rollout and a
gradient-times-attention product, while LeGrad-HuBERT uses positive attention
gradients directly and does not use rollout.

\subsubsection*{Decisions made}
\begin{itemize}
  \item The Level 3 workflow is kept in its own notebook so it is not confused
  with the LeGrad-HuBERT implementation.
  \item The notebook exposes the intermediate tensors
  \texttt{grad\_attentions}, \texttt{joint\_attention},
  \texttt{rollout\_score}, \texttt{head\_relevance}, and
  \texttt{scores["level3"]}.
  \item The final evaluation score remains sum-normalized, while a min-max
  score is used only for visual contrast in plots.
  \item The same utterance, \texttt{Ses01F\_impro01\_F005}, is used so the
  LeGrad-HuBERT and Level 3 notebooks can be compared directly.
\end{itemize}

\subsubsection*{What was tested}
Notebook 05 was executed on \texttt{Ses01F\_impro01\_F005}. It loaded the
HuBERT classifier, captured forward attentions and classifier-head inputs,
computed target-class attention gradients, built all layer-level
gradient-times-attention matrices, computed rollout and head relevance, and
verified that a manual reconstruction of
\texttt{normalize(rollout\_score * head\_relevance)} exactly matched
\texttt{scores["level3"]}.

\subsubsection*{Outcome}
The notebook completed successfully. The selected utterance was predicted as
\texttt{ang}, produced 12 Transformer attention layers and 201 temporal tokens,
and the maximum absolute difference between the manual Level 3 reconstruction
and the project implementation was 0.0. The notebook also displays top temporal
tokens and renders both the spectrogram relevance timeline and the
Notebook-01-style waveform interval plot.

\subsubsection*{Remaining issues}
Level 3 remains a hybrid rollout baseline rather than a LeGrad reproduction.
The same uniform token-to-time mapping approximation used elsewhere in the
project also applies here.

\subsection{Notebook 06 --- Quantitative Evaluation}
...
\subsubsection*{Technical idea}
The validated SpeechXAI workflow from Notebook 01, LeGrad-HuBERT workflow from
Notebook 04, and Level 3 workflow from Notebook 05 are applied to the same
controlled evaluation protocol. SpeechXAI top-$k$ words define a union duration
\(X\). SpeechXAI masks, HuBERT explanation top bins, and repeated random bins
are then compared after silencing exactly \(X\) seconds while preserving
waveform length.
...
\begin{itemize}
  ...
  \item HuBERT explanation bins are selected by score until their union duration reaches $X$.
...
\subsubsection*{What was tested}
Notebook 01 validated real SpeechXAI attribution, silence masking,
union-duration accounting, confidence drop, and repeated same-duration random
masking on one utterance. The consolidated Notebook 06 contains the protocol
scaffold that extends those validated operations to SpeechXAI, Level 3, and
LeGrad-HuBERT modes across the evaluation set.
...
\subsubsection*{Remaining issues}
The complete run requires the working LeGrad-HuBERT example from Notebook 04,
the Level 3 example from Notebook 05, a documented evaluation set, and execution
across all selected audios.

\subsection{Notebook 07 --- Qualitative Examples}
...
\subsubsection*{Remaining issues}
Representative cases can be selected after Notebook 06 produces the complete
deletion results. Aggregate tables remain in Notebook 06; this notebook is
reserved for case-level interpretation.
...
\begin{enumerate}
  ...
  \item HuBERT explanation time bins are ranked by relevance, and the highest-ranked bins whose total duration equals $X$ are selected.
  \item Those HuBERT explanation regions are replaced by silence, and the same confidence drop is measured.
