...
\]
The implementation follows the SpeechXAI repository behavior by applying the word mask in the waveform domain with the same effective interval logic used in our notebooks and scripts. This produces word scores, ranked word intervals, and a human-readable explanation.
...
\textbf{Duration-matched deletion evaluation.} For each audio and each \(k\in\{1,2,3,5\}\), Pastor/SpeechXAI top-\(k\) words define a total removed duration \(X\). Level 3 and LeGrad-HuBERT then select their top HuBERT token intervals until the same duration \(X\) is reached. All methods silence their selected intervals without changing waveform length. The main metric is the confidence drop for the original predicted class:
\[
\Delta p = p(\hat{y}\mid x)-p(\hat{y}\mid x_{\mathrm{masked}}).
\]
Higher \(\Delta p\) means the selected region was more important for the model's original decision. A random deletion by silence masking baseline selects duration-matched random bins and runs 20 trials per audio.
...
\bottomrule
\end{tabular}
\caption{Duration-matched deletion results. Pastor/SpeechXAI uses word intervals; Level 3 and LeGrad-HuBERT use HuBERT token intervals selected to match the same removed duration.}
