...
\subsubsection*{Technical idea}
LeGrad uses sensitivity to attention maps as its explanation signal. For speech, the target quantity is the classifier score for the original predicted emotion, and the required diagnostic is $\partial s_{\hat{y}}/\partial A_l$ for one or more Transformer layers. The later implementation in this repository uses those gradients in a modified way by multiplying them with attention values before rollout.
...
\subsubsection*{Technical idea}
For each Transformer layer, positive class-sensitive attention gradients are combined with attention activations, averaged across heads, propagated with rollout, and reduced to token relevance. Token relevance is then mapped to audio time. This is a LeGrad-inspired adaptation rather than the original LeGrad reduction, which uses the positive attention gradient itself as the explanation signal.

\subsubsection*{Decisions made}
\begin{itemize}
  \item The method is presented as LeGrad-inspired because it adapts the attention-gradient sensitivity principle from vision to speech.
  \item Relevance uses positive gradients multiplied by attention values before rollout, which differs from original LeGrad.
...
LeGrad-inspired formulation & Positive attention gradients multiplied by attention values, then used in rollout & This borrows LeGrad's attention-gradient sensitivity but is not the original LeGrad reduction. \\
